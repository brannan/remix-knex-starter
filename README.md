...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create indie-stack-6a02
  fly apps create indie-stack-6a02-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app indie-stack-6a02
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app indie-stack-6a02-staging
  ```
...
- Create a persistent volume for the sqlite database for both your staging and
  ...
  ```sh
  fly volumes create data --size 1 --app indie-stack-6a02
  fly volumes create data --size 1 --app indie-stack-6a02-staging
  ```
...
(including your app name) to [the Fly support
community](https://community.fly.io). They're normally pretty responsive over
there and hopefully can help resolve any of your deployment issues and
questions.
...
auto-complete. To run type checking across the whole project, run `npm run
typecheck`.
...
It's recommended to install an editor plugin (like the [VSCode Prettier
